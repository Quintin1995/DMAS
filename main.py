...
def get_agent_codename (agent_idx): 
    return "Agent {}".format("ABCDEFGHIJKLMNOPQRSTUVWXYZ"[agent_idx])

def get_secret_codename (secret_idx):
    return "secret {}".format("abcdefghijklmnopqrstuvwxyz"[secret_idx])
...
for target_idx in range (AMOUNT_AGENTS):
    for agent_idx in range (AMOUNT_AGENTS):
        print ("{} thinks {} has secret {}".format(get_agent_codename(agent_idx), get_agent_codename(target_idx), get_secret_codename(get_secret_value(agent_idx, target_idx))))
